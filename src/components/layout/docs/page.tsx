...
import { AnchorProvider, useActiveAnchors, type TOCItemType } from "fumadocs-core/toc";
  ...
  return (
    <AnchorProvider toc={toc}>
      <main className="flex w-full min-w-0 flex-col">
        <article className="flex flex-1 flex-col w-full max-w-215 gap-6 px-4 py-8 md:px-6 md:mx-auto">
          {props.children}
          <Footer />
        </article>
      </main>
      {toc.length > 0 && (
        <div className="sticky top-(--fd-nav-height) w-71.5 shrink-0 h-[calc(100dvh-var(--fd-nav-height))] p-4 overflow-auto max-xl:hidden">
