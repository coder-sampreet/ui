...
import { createContext, use, useMemo, useState, type ComponentProps, type ReactNode } from "react";
...
function SearchToggle(props: ComponentProps<"button">) {
  ...
  return (
    <button {...props} className={cn("text-sm", props.className)} onClick={() => setOpenSearch(true)}>
      Search
    </button>
  );
}
...
function Sidebar() {
  ...
  return (
    <aside
      className={cn(
        "fixed flex flex-col shrink-0 p-4 top-14 z-20 text-sm overflow-auto md:sticky md:h-[calc(100dvh-56px)] md:w-75",
